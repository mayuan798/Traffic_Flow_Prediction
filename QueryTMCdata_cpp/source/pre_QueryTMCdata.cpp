...
int main(int argc, char* argv[])
{
	...
	path root ("D:\Programming\Github\Deep_Learning_Project_Traffilc_Flow_Prediction\root_data");
	...
	try{
		if (exists(p)){    // does p actually exist?
		  if (is_directory(p)){      // is p a directory?
			...
			if (boost::regex_match(p.string(), folder_filter)){	// cicumstance 2: target folder
				clock_t begin_time = std::clock();
				// create a file stream
				std::cout << "******************************" << endl;
				ofstream Output_file (p.string() + "test_TMC_output.csv");			// Opening file to print info to
			...
			else {				// cicumstance 3: folder but not target folder
				for (vector<path>::const_iterator it(v.begin()), it_end(v.end()); it != it_end; ++it){
					cout << "   " << *it << '\n';
